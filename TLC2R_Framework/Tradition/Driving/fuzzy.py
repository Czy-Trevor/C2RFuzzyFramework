...
class Fuzzy:
    def __init__(self,fs_num=3, ys = None, FSS = False):
        ...
        self.FSS = FSS
        '''
        Decide whether to use fuzzy set selection. If FSS==True, the fuzzy sets are defined based on the theoretical label intervals of the dataset.
        '''
        if self.FSS == False:
            self.min = np.min(ys)
            self.max = np.max(ys)
            ...
            self.p5 = 0.05
            self.p95 = 0.95
            self.triband = np.arange(0.05,0.95,(0.95 - 0.05)/(self.num - 2 + 1))
