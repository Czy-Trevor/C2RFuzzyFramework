...
np.random.seed(0)
loaded_x = np.load('/mnt/data/zycui/DATA/PVTpsd.npz')
loaded_y = np.load('/mnt/data/zycui/DATA/PVTlabel.npz')
...
def train_all(T=10, SDAR=True, KR=1, dim=62, max_fs=10):
    sub_num = len(X)
    start_time = time.time()

    result = Parallel(n_jobs=1)(delayed(target_train)(i, T, SDAR, KR, dim, max_fs, X, Y) for i in range(sub_num))
